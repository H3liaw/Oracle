...
contract SharePriceOracle is ISharePriceOracle, OwnableRoles {
        ...
        returns (VaultReport[] memory reports)
    {
        ...
        for (uint256 i = 0; i < len; i++) {
            ...
            uint256 sharePrice = vault.convertToAssets(10 ** vaultDecimals);
        ...
        returns (uint256 sharePrice, uint64 timestamp)
    {
        ...
        if (_srcChainId == chainId) {
            ...
            uint8 assetDecimals = _getAssetDecimals(asset);
            uint8 vaultDecimals = vault.decimals();
            sharePrice = vault.convertToAssets(10 ** vaultDecimals);
    ...
    /**
     * @notice Gets the price key for a vault
     * @param _chainId The chain ID
     * @param vaultAddress The vault address
     * @return The price key
     */
