...
contract PythAdapterTest is Test {
    ...
    uint32 constant AGE = 3600; // 1 hour
    ...
    function setUp() public {
        ...
        adapter.addAsset(WETH, ETH_USD_PRICE_ID, AGE, MAX_AGE, true);

        adapter.addAsset(WBTC, BTC_USD_PRICE_ID, AGE, MAX_AGE, true);
    ...
    function testPriceError_StalePrice() public {
        ...
        PythStructs.Price memory stalePrice = PythStructs.Price({
            ...
            conf: 5_000_000,
            expo: -8,
            publishTime: uint64(block.timestamp - AGE - 1)
        });
    ...
    function testRevertAddAsset_InvalidHeartbeat() public {
        uint32 invalidHeartbeat = adapter.AGE() + 1;
    ...
    function testRevertAddAsset_InvalidMaxAge() public {
        ...
        adapter.addAsset(USDC, bytes32(0), AGE, invalidMaxAge, true);
    }
