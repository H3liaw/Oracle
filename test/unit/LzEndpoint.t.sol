...
contract MaxLzEndpointTest is BaseTest {
    ...
    function setUp() public {
        ...
        // Configure price feeds in router
        oracle_pol.setLocalAssetConfig(WETH_POLYGON, address(polygonAdapter), POLYGON_ETH_USD_FEED, 0, true);
        oracle_pol.setLocalAssetConfig(WBTC_POLYGON, address(polygonAdapter), POLYGON_BTC_USD_FEED, 0, true);
        ...
        oracle.setCrossChainAssetMapping(137, WETH_POLYGON, WETH);
        oracle.setCrossChainAssetMapping(137, WBTC_POLYGON, WBTC);
        ...
        inUSD[0] = true;
        inUSD[1] = true;
        oracle.batchUpdatePrices(assets, inUSD);
        ...
        oracle_pol.setCrossChainAssetMapping(8453, WETH, WETH_POLYGON);
        oracle_pol.setCrossChainAssetMapping(8453, WBTC, WBTC_POLYGON);

        address[] memory assets_pol = new address[](2);
        assets_pol[0] = WETH_POLYGON;
        assets_pol[1] = WBTC_POLYGON;
        bool[] memory inUSD_pol = new bool[](2);
        inUSD_pol[0] = true;
        inUSD_pol[1] = true;
        oracle_pol.batchUpdatePrices(assets_pol, inUSD_pol);
    ...
    function test_sendSharePrices_PolygonToBase() public {
        switchTo("BASE");
        vm.startPrank(user);
        ...
        uint256 Fee = endpoint_pol.estimateFees(30_109, 1, abi.encode(1, vaults, user), options);
        ...
        endpoint_pol.sendSharePrices{ value: Fee * 5 }(30_109, vaults, options, user);
    ...
    function test_requestSharePrices_BaseToPolygon() public {
        ...
        ISharePriceOracle.VaultReport[] memory reports = oracle.getSharePrices(vaults, user);
        ...
        ISharePriceOracle.VaultReport memory report = oracle_pol.getLatestSharePriceReport(8453, address(vault));

        (uint248 price, uint8 decimals, uint64 timestamp,) = oracle_pol.storedSharePrices(address(vault));

        assertEq(price, report.sharePrice);
        assertEq(decimals, report.assetDecimals);
        assertEq(timestamp, report.lastUpdate);
        assertEq(report.vaultAddress, address(vault));
